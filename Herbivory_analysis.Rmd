...
```{r Load and head}
herbivory <- read.table(file = "C:/Users/Granjel/Desktop/Nico_Gross/Herbivory_2013.txt", header = TRUE, sep = "\t")
head(herbivory)
```
...
library(nlme)
Grasshoppers_linear <- lme(Total ~ Cb + Cd + Ci + Ee + Pg + Pp, data = herbivory, random = ~ 1 | block)
...
Grasshoppers_quadratic <- lme(Total ~ (Cb)^2 + (Cd)^2 + (Ci)^2 + (Ee)^2 + (Pg)^2 + (Pp)^2, data = herbivory, random = ~ 1 | block)
...
Grasshoppers_log <- lme(Total ~ log(Cb+1) + log(Cd+1) + log(Ci+1) + log(Ee+1) + log(Pg+1) + log(Pp+1), data = herbivory, random = ~ 1 | block)
...
Grasshoppers_exp <- lme(Total ~ exp(Cb) + exp(Cd) + exp(Ci) + exp(Ee) + exp(Pg) + exp(Pp), data = herbivory, random = ~ 1 | block)
...
```{r Linear model Cb}
Cb_linear <- lme(Total ~ Cb, data = herbivory, random = ~ 1 | block)
summary(Cb_linear)
```
...
Cb_quadratic <- lme(Total ~ (Cb)^2, data = herbivory, random = ~ 1 | block)
...
Cb_log <- lme(Total ~ log(Cb+1), data = herbivory, random = ~ 1 | block)
summary(Cb_log)
```
...
Cb_exp <- lme(Total ~ exp(Cb), data = herbivory, random = ~ 1 | block)
summary(Cb_exp)
```
...
```{r Linear model Cd}
Cd_linear <- lme(Total ~ Cd, data = herbivory, random = ~ 1 | block)
summary(Cd_linear)
```
...
Cd_quadratic <- lme(Total ~ (Cd)^2, data = herbivory, random = ~ 1 | block)
...
Cd_log <- lme(Total ~ log(Cd+1), data = herbivory, random = ~ 1 | block)
summary(Cd_log)
```
...
Cd_exp <- lme(Total ~ exp(Cd), data = herbivory, random = ~ 1 | block)
summary(Cd_exp)
```
...
```{r Linear model Ci}
Ci_linear <- lme(Total ~ Ci, data = herbivory, random = ~ 1 | block)
summary(Ci_linear)
```
...
Ci_quadratic <- lme(Total ~ (Ci)^2, data = herbivory, random = ~ 1 | block)
...
Ci_log <- lme(Total ~ log(Ci+1), data = herbivory, random = ~ 1 | block)
summary(Ci_log)
```
...
Ci_exp <- lme(Total ~ exp(Ci), data = herbivory, random = ~ 1 | block)
summary(Ci_exp)
```
...
```{r Linear model Ee}
Ee_linear <- lme(Total ~ Ee, data = herbivory, random = ~ 1 | block)
summary(Ee_linear)
```
...
Ee_quadratic <- lme(Total ~ (Ee)^2, data = herbivory, random = ~ 1 | block)
...
Ee_log <- lme(Total ~ log(Ee+1), data = herbivory, random = ~ 1 | block)
summary(Ee_log)
```
...
Ee_exp <- lme(Total ~ exp(Ee), data = herbivory, random = ~ 1 | block)
summary(Ee_exp)
```
...
```{r Linear model Pg}
Pg_linear <- lme(Total ~ Pg, data = herbivory, random = ~ 1 | block)
summary(Pg_linear)
```
...
Pg_quadratic <- lme(Total ~ (Pg)^2, data = herbivory, random = ~ 1 | block)
...
Pg_log <- lme(Total ~ log(Pg+1), data = herbivory, random = ~ 1 | block)
summary(Pg_log)
```
...
Pg_exp <- lme(Total ~ exp(Pg), data = herbivory, random = ~ 1 | block)
summary(Pg_exp)
```
...
```{r Linear model Pp}
Pp_linear <- lme(Total ~ Pp, data = herbivory, random = ~ 1 | block)
summary(Pp_linear)
```
...
Pp_quadratic <- lme(Total ~ (Pp)^2, data = herbivory, random = ~ 1 | block)
...
Pp_log <- lme(Total ~ log(Pp+1), data = herbivory, random = ~ 1 | block)
summary(Pp_log)
```
...
Pp_exp <- lme(Total ~ exp(Pp), data = herbivory, random = ~ 1 | block)
summary(Pp_exp)
```
