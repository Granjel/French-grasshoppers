...
```{r BROERE lme}
broere_lme <- gamm(d_BROERE ~ s(BROERE) + s(ARRELA) + s(DACGLO) + s(DAUCAR) + s(PLALAN) + s(POAANG),
                   data = data_BROERE, random = list(block = ~1), family = poisson,
                   correlation= corAR1(form= ~ d_time| unique)) # poisson or quasipoisson?
broere_lme
```
