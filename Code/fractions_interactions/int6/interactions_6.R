...
alphas <- as.data.frame(read.table("PATH/int6/interactions_alphas4.txt", header = TRUE, sep = "\t")) ########################## CHANGE
intr <- as.data.frame(read.table("PATH/int6/interactions_intrinsic4.txt", header = TRUE, sep = "\t")) ########################## CHANGE
...
interactions <- foreach (i = 81:96, .combine = rbind, .packages='mvtnorm') %dopar% { #you need to say which packages you are using
...
#CHANGE PATH:
write.table(interactions, file = "PATH/int6/structural_coex_interactions_6.txt", sep = "\t", row.names = FALSE)
