...
alphas <- as.data.frame(read.table("PATH/int1/interactions_alphas4.txt", header = TRUE, sep = "\t")) ########################## CHANGE
intr <- as.data.frame(read.table("PATH/int1/interactions_intrinsic4.txt", header = TRUE, sep = "\t")) ########################## CHANGE
...
interactions <- foreach (i = 1:16, .combine = rbind, .packages='mvtnorm') %dopar% { #you need to say which packages you are using
...
#CHANGE PATH:
write.table(interactions, file = "PATH/int1/structural_coex_interactions_1.txt", sep = "\t", row.names = FALSE)
