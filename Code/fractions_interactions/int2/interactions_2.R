...
alphas <- as.data.frame(read.table("PATH/int2/interactions_alphas4.txt", header = TRUE, sep = "\t")) ########################## CHANGE
intr <- as.data.frame(read.table("PATH/int2/interactions_intrinsic4.txt", header = TRUE, sep = "\t")) ########################## CHANGE
...
interactions <- foreach (i = 17:32, .combine = rbind, .packages='mvtnorm') %dopar% { #you need to say which packages you are using
...
#CHANGE PATH:
write.table(interactions, file = "PATH/int2/structural_coex_interactions_2.txt", sep = "\t", row.names = FALSE)
