...
alphas <- as.data.frame(read.table("Results/interactions_alphas3.txt", header = TRUE, sep = "\t")) ########################## CHANGE
intr <- as.data.frame(read.table("Results/interactions_intrinsic3.txt", header = TRUE, sep = "\t")) ########################## CHANGE
...
interactions_sobremesa <- foreach (i = 1:length(alphas), .combine = rbind, .packages='mvtnorm') %dopar% { #you need to say which packages you are using
...
#CHANGE PATH:
write.table(interactions_sobremesa, file = "Results/structural_coex_interactions_sobremesa.txt", sep = "\t", row.names = FALSE)
