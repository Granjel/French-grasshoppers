...
alphas <- as.data.frame(read.table("C:/Users/teres/Desktop/Scripts_R/interactions_alphas1.txt", header = TRUE, sep = "\t")) ########################## CHANGE
alphas <- list_from_df(alphas, 22)


#working
nspecies <- 22
intr <- as.data.frame(read.table("C:/Users/teres/Desktop/Scripts_R/interactions_intrinsic1.txt", header = TRUE, sep = "\t")) ########################## CHANGE
...
interactions_Teresa <- foreach (i = 1:length(alphas), .combine = rbind, .packages='mvtnorm') %dopar% { #you need to say which packages you are using
...
write.table(interactions_Teresa, file = "C:/Users/teres/Desktop/Scripts_R/structural_coex_interactions_Teresa.txt", sep = "\t", row.names = FALSE)
